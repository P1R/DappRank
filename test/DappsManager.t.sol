...
contract DappsManagerTest is Test {
    ...
    function testBuyTwiceNoInflation() public {
        address buyer = address(0x77);
        vm.deal(buyer, 1 ether);

        // First buy: new fan -> msg.value * 1000
        vm.prank(buyer);
        dappsMgr.buyDRNK{value: 0.01 ether}();
        assertEq(drnkToken.balanceOf(buyer), 0.01 ether * 1000); // 10 DRNK

        // Second buy: existing fan -> multiplier(1) * msg.value * 1000.
        // Regression: multiplier used to start at `bonus`, minting
        // bonus * value * 1000 (astronomical) on the 2nd buy.
        vm.prank(buyer);
        dappsMgr.buyDRNK{value: 0.01 ether}();
        assertEq(drnkToken.balanceOf(buyer), 0.02 ether * 1000); // 20 DRNK total
    }
    ...
    // --- The Graph integration: event emission tests ---
    // PROPUESTA PENDIENTE DE APROBACIÓN: descomentar junto con los eventos en
    // src-sc/DappsManager.sol (los eventos aún no están aprobados).

    // function testRegisterDappEmitsEvent() public {
    //     bytes32 dappName = "TestDapp";
    //     string memory cid = "QmTestCID";
    //
    //     vm.expectEmit(true, true, true, true);
    //     emit DappsManager.DappRegistered(dappName, address(this), cid);
    //     dappsMgr.registerDapp{value: listingFee}(dappName, cid);
    // }
    //
    // function testApproveDappEmitsEvent() public {
    //     bytes32 dappName = "TestDapp";
    //     dappsMgr.registerDapp{value: listingFee}(dappName, "QmTestCID");
    //
    ...
    // }
    //
    // function testBanDappEmitsEvent() public {
    //     bytes32 dappName = "TestDapp";
    //     dappsMgr.registerDapp{value: listingFee}(dappName, "QmTestCID");
    //
    //     vm.expectEmit(true, true, false, false);
    ...
    // }
    //
    // function testVoteDappEmitsEvents() public {
    //     dappsMgr.demoAirdrop(testUsers);
    //     bytes32 dappName = "dapp1";
    //     vm.prank(testUsers[0]);
    //     vm.deal(testUsers[0], 10 ether);
    //     dappsMgr.registerDapp{value: listingFee}(dappName, "QmNewCID");
    //     dappsMgr.approveDapp(dappName);
    //
    //     uint256 voteAmount = 100e18;
    //     vm.prank(testUsers[0]);
    //     drnkToken.approve(address(dappsMgr), voteAmount);
    //
    //     uint256 burnAmount = (voteAmount * burningFee) / 10_000; // 10e18
    //
    //     // VoteCast: check indexed topics (dapp, voter), skip data
    //     vm.expectEmit(true, true, true, false);
    //     emit DappsManager.VoteCast(dappName, testUsers[0], 0, 0, 0, 0);
    //     // TokensBurned: check dapp topic + amount data
    //     vm.expectEmit(true, true, false, true);
    //     emit DappsManager.TokensBurned(dappName, burnAmount);
    //
    //     vm.prank(testUsers[0]);
    //     dappsMgr.voteDapp(dappName, voteAmount, 1);
    // }
    //
    // function testCashOutEmitsEventAndUpdatesBalance() public {
    //     dappsMgr.demoAirdrop(testUsers);
    //     bytes32 dappName = "dapp1";
    //     vm.prank(testUsers[0]);
    //     vm.deal(testUsers[0], 10 ether);
    ...
    //     drnkToken.approve(address(dappsMgr), voteAmount);
    //     vm.prank(testUsers[0]);
    //     dappsMgr.voteDapp(dappName, voteAmount, 50);
    //
    //     (, , , , uint256 balanceBefore, , , ) = dappsMgr.getDappInfo(dappName);
    //
    //     vm.expectEmit(true, true, true, true);
    //     emit DappsManager.DappCashOut(dappName, testUsers[0], 50e18);
    //     vm.prank(testUsers[0]);
    //     dappsMgr.dappCashOut(dappName, 50e18);
    //
    //     (, , , , uint256 balanceAfter, , , ) = dappsMgr.getDappInfo(dappName);
    //     assertEq(
    //         balanceAfter,
    //         balanceBefore - 50e18,
    //         "balance should decrease by cashout amount"
    //     );
    // }
    //
    // function testUpdateDappCIDEmitsEvent() public {
    //     bytes32 dappName = "TestDapp";
    //     dappsMgr.registerDapp{value: listingFee}(dappName, "QmTestCID");
    //
    //     vm.expectEmit(true, true, true, true);
    //     emit DappsManager.DappCIDUpdated(dappName, "QmNewCID");
    //     dappsMgr.updateDappCID(dappName, "QmNewCID");
    // }
    //
    // function testRemoveDappEmitsEvent() public {
    //     bytes32 dappName = "TestDapp";
    //     dappsMgr.registerDapp{value: listingFee}(dappName, "QmTestCID");
    //
    //     vm.expectEmit(true, true, false, false);
